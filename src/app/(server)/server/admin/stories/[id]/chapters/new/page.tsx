import { getAuthFromCookie, hasRole } from "@/features/auth/server";
import { getStoryById } from "@/features/admin/server";
import { redirect, notFound } from "next/navigation";
import type { Metadata } from "next";
import { NewChapterPageClient } from "./new-chapter-page-client";

interface NewChapterPageProps {
  params: Promise<{ id: string }>;
}

export async function generateMetadata({
  params,
}: NewChapterPageProps): Promise<Metadata> {
  const { id } = await params;
  const storyResult = await getStoryById(id);

  if (!storyResult.success) {
    return { title: "Không tìm thấy truyện | Nekozanedex Admin" };
  }

  return {
    title: `Thêm chapter: ${storyResult.data.title} | Nekozanedex Admin`,
    description: `Thêm chapter mới cho ${storyResult.data.title}`,
  };
}

export default async function NewChapterPage({ params }: NewChapterPageProps) {
  const { id } = await params;

  // Auth check
  const authResult = await getAuthFromCookie();
  if (!authResult.success || !hasRole(authResult.data, ["admin"])) {
    redirect("/auth/login");
  }

  // Fetch story
  const storyResult = await getStoryById(id);
  if (!storyResult.success) {
    notFound();
  }

  return <NewChapterPageClient story={storyResult.data} />;
}
